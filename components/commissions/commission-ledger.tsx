...
function metricsFor(scope: CommissionLedgerScope, entries: CommissionLedgerEntry[]) {
  ...
  if (scope === "partner") {
    return [
      ...
      { label: "Team payouts", value: sum(teamEntries, () => true), icon: WalletCards, info: "Commissions owed to managers, leaders, and agents." },
  ...
  if (scope === "manager") {
    return [
      ...
      { label: "Group payouts", value: sum(teamEntries, () => true), icon: WalletCards, info: "Commissions owed to leaders and agents under you." },
      { label: "Pending", value: pending, icon: Clock3, info: "Your commissions that are calculated but not ready or marked paid yet." },
      { label: "Approved", value: approved + paid, icon: CheckCircle2, info: "Your commissions approved or already marked paid." }
    ];
  }
  ...
  if (scope === "group_leader") {
    return [
      ...
      { label: "Agent payouts", value: sum(teamEntries, () => true), icon: WalletCards, info: "Commissions owed to agents under you." },
      { label: "Pending", value: pending, icon: Clock3, info: "Your commissions that are calculated but not ready or marked paid yet." },
      { label: "Approved", value: approved + paid, icon: CheckCircle2, info: "Your commissions approved or already marked paid." }
    ];
  }
...
function helperCopy(scope: CommissionLedgerScope) {
  if (scope === "admin") {
    return "Company-wide ledger across partner pools, manager and leader overrides, agent commissions, approvals, deferrals, and paid payouts.";
  }

  if (scope === "partner") {
    return "Your partner profit and the payout obligations generated by your managers, leaders, and agents.";
  }
  ...
  if (scope === "group_leader") {
    return "Your personal earnings plus agent commission activity for the agents assigned to your leader profile.";
  }
    ...
    <div className="space-y-6">
      ...
      <RecordFilters
        title="Commission filters"
            description="Filter commission activity by customer, order, agent, payout recipient, status, or role."
